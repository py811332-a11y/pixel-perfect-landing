...
import { Brain, RefreshCw, Share2, BookOpen } from "lucide-react";
    ...
    <AppLayout>
      <div className="p-6 lg:p-8 max-w-3xl mx-auto">
          ...
          {groupTestResults.map((r) => (
            <Card key={r.rank} className={r.isYou ? "border-primary" : ""}>
              <CardContent className="p-4 flex items-center gap-4">
                <span className="text-lg font-display font-bold w-8 text-center text-muted-foreground">{r.badge || r.rank}</span>
        ...
        <Card className="mt-6 bg-primary/5 border-primary/20">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-semibold text-foreground">AI Group Insight</p>
            </div>
        ...
        <div className="flex gap-3 mt-6">
          <Link to="/group-test/create"><Button variant="outline" className="gap-2"><RefreshCw className="w-4 h-4" /> Play Again</Button></Link>
          <Button variant="outline" className="gap-2"><Share2 className="w-4 h-4" /> Share Result</Button>
          <Link to="/subjects/science"><Button className="gap-2"><BookOpen className="w-4 h-4" /> Study This Topic</Button></Link>
        </div>
      </div>
    </AppLayout>
  );
