        ...
        <div className="flex gap-2 mb-8">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${tab === t.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{t.label}</button>
          ))}
        </div>
        ...
        <div className="flex justify-center items-end gap-4 mb-8">
          {[leaderboard[1], leaderboard[0], leaderboard[2]].map((entry, i) => (
            <div key={entry.rank} className={`text-center ${i === 1 ? "order-2" : i === 0 ? "order-1" : "order-3"}`}>
              <div className={`w-16 h-16 rounded-full bg-muted flex items-center justify-center text-2xl mx-auto mb-2 ${i === 1 ? "ring-4 ring-accent" : ""}`}>
                {entry.badge}
              </div>
        ...
        <div className="space-y-2">
          {leaderboard.slice(3).map(entry => (
            <div key={entry.rank} className={`flex items-center gap-3 p-3 rounded-lg ${entry.isYou ? "bg-primary/5 border border-primary/20" : "hover:bg-muted"}`}>
              ...
              <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-sm">{entry.name[0]}</div>
